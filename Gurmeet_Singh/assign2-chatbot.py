...
client = OpenAI(
    ...
    api_key=api_key
    # optional: shows on OpenRouter rankings
...
if "openai_model" not in st.session_state:
    st.session_state["openai_model"] = "openai/gpt-4o-mini"; #"openai/gpt-oss-120b"
...
def generate_summary(messages):
    # 1. Flatten the conversation into a single string
    conversation_text = ""
    for msg in messages:
        if msg["role"] in ("user", "assistant"):
    ...
    # 2. Prompt the LLM
    prompt = f"Summarize the following conversation in 2-3 concise bullet points:\n\n{conversation_text}"
    #api_messages = [{"role": "system", "content": system_message}]
    api_messages = [{"role": "user", "content": prompt}]

    # 3. Call your model (Example using Gemini)
...
if st.checkbox("Show Development Info", value=False):
    ...
    st.write(f"**Coversation Summary:** {generate_summary(st.session_state.messages)}")